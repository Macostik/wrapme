...
final class HomeViewController: BaseViewController {
    ...
    override func viewDidLoad() {
        ...
        self.homeDataSource.autogeneratedMetrics.change {
        ...
        self.publicDataSource.autogeneratedMetrics.selection = self.homeDataSource.autogeneratedMetrics.selection
        
        self.publicDataSource.autogeneratedMetrics.modifyItem = { $0.insets = CGRectMake(0, $0.position.index == 0 ? 5 : 0, 0, 0) }
        
        self.homeDataSource.addMetrics(RecentCandiesView.layoutMetrics().change({ [weak self] metrics in
            
            metrics.modifyItem = { item in
                let size = (Constants.screenWidth - 2.0)/3.0
                item.size = (self?.homeDataSource.wrap?.candies.count > Constants.recentCandiesLimit_2 ? 2*size : size) + 5
        ...
        self.homeDataSource.autogeneratedMetrics.finalizeAppearing = { [weak self] item, view in
            self?.photoButton.hidden = false
        }
        self.publicDataSource.autogeneratedMetrics.finalizeAppearing = self.homeDataSource.autogeneratedMetrics.finalizeAppearing
        
        self.homeDataSource.placeholderMetrics = StreamMetrics(loader: HomePlaceholderView.homePlaceholderLoader({ [weak self] () -> Void in
        ...
        self.publicDataSource.loadingMetrics.modifyItem = { [weak self] (item) in
            item.size = (self?.streamView.height ?? 0) - (self?.publicWrapsHeaderView.height ?? 0.0) - 48
        }
        ...
        self.homeDataSource.items = PaginatedList(entries:Array(wraps), request:PaginatedRequest.wraps(nil))
        
        if !wraps.isEmpty {
            self.homeDataSource.refresh()
        }
