...
@implementation UploadSummaryViewController
...
- (void)viewDidLoad {
    ...
    [self.dataSource.streamView scrollToItemPassingTest:^BOOL(StreamItem *item) {
        return item.entry == self.assets.lastObject;
    } animated:NO];
    [self setViewController:[self editAssetViewControllerForAsset:self.assets.lastObject] direction:0 animated:NO];
    
    __weak typeof(self)weakSelf = self;
    [dataSource.autogeneratedMetrics setSelection:^(StreamItem *item, MutableAsset *asset) {
        if (weakSelf.asset != asset) {
            [weakSelf setViewController:[weakSelf editAssetViewControllerForAsset:asset] direction:0 animated:NO];
        }
    }];
}
...
- (EditAssetViewController *)editAssetViewControllerForAsset:(MutableAsset *)asset {
...
- (UIViewController *)viewControllerAfterViewController:(EditAssetViewController *)viewController {
    ...
    if (picture) {
        return [self editAssetViewControllerForAsset:picture];
    }
    return nil;
}
...
- (IBAction)upload:(id)sender {
    ...
    [self.delegate uploadSummaryViewController:self didFinishWithAssets:self.assets];
}
...
- (IBAction)deletePicture:(id)sender {
    ...
    [self.delegate uploadSummaryViewController:self didDeselectAsset:self.asset];
    if (assets.count > 0) {
        self.assets = [NSArray arrayWithArray:assets];
        self.dataSource.items = self.assets;
        if (index < assets.count) {
            self.asset = [assets tryAt:index];
            [self setViewController:[self editAssetViewControllerForAsset:self.asset] direction:WLSwipeViewControllerDirectionForward animated:YES];
        } else {
            self.asset = [assets tryAt:index - 1];
            [self setViewController:[self editAssetViewControllerForAsset:self.asset] direction:WLSwipeViewControllerDirectionReverse animated:YES];
        }
    } else {
        [self back];
    }
