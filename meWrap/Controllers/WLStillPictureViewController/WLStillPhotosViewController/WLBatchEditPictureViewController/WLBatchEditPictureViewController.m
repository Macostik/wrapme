...
@property (weak, nonatomic) MutableAsset *picture;
...
- (void)viewDidLoad {
    ...
    [dataSource.autogeneratedMetrics setSelection:^(StreamItem *item, MutableAsset *picture) {
...
- (void)setPicture:(MutableAsset *)picture {
    _picture = picture;
    [self updatePictureData:picture];
}

- (void)updatePictureData:(MutableAsset *)picture {
    if (picture.deleted) {
        ...
        self.drawButton.hidden = self.editButton.hidden = picture.type == MediaTypeVideo;
        self.restoreButton.hidden = YES;
        self.composeBar.text = picture.comment;
        if (picture.type == MediaTypeVideo) {
...
- (UIViewController *)viewControllerAfterViewController:(WLEditPictureViewController *)viewController {
    MutableAsset *picture = [self.pictures tryAt:[self.pictures indexOfObject:viewController.picture] + 1];
    if (picture) {
